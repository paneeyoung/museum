        ...
        <div className={modalWrapperClass} onClick={() => setIsOpen(false)}>
          <div onClick={(e) => e.stopPropagation()} className={modalCardClass}>
            <form key={resetKey} action={formAction} className="flex flex-col gap-3">
              ...
              {/* Every existing shift is capacity 1 — multi-person coverage is
                  handled by separately-named shifts (e.g. "Baliemedewerker
                  01/02") instead, since capacity>1 has a known unfixed
                  ambiguity in Auto-plan's fill order. No UI for it. */}
